...
# ==========================================
st.header("1. 기초 회계 관리: 월별 예산 집행 및 계정별 잔액 트래킹")

st.markdown("""
* **재무회계 실무 적용:** Series A 투자 유치 이후, 알츠하이머 AI 진단 기술 고도화를 위한 **R&D 연구개발비와 IT/서버 인프라 비용**이 증가하고 있습니다. 
단순 수익 지표가 아닌, 부서별 지출결의 현황과 계정별 가용 예산 잔액을 시각화하여 경영진의 자금 운용 의사결정을 돕고, 
전표 입력 및 월 마감(결산) 리드타임을 단축하기 위해 현금 흐름을 실시간으로 모니터링합니다.
""")
...
st.markdown("IT/소프트웨어 업종 특성을 반영하여 파이썬(Pandas)으로 증빙 데이터를 1원 단위까지 크로스체크합니다.")
