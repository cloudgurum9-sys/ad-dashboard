...
st.markdown("""
* **재무회계 실무 적용:** YBM의 비즈니스 특성상 매일 발생하는 수많은 강의 수강료 및 시험 응시료 결제액을 '선수수익(부채)'으로 우선 인식한 후, 수강 기간 경과 및 시험일 도래에 따라 **'매출'로 대체**하는 결산 과정이 핵심이라고 생각합니다.
...
with col1:
    # 4월 기준으로 변경
    st.metric(label="4월 총 결제발생액 (B2C)", value=f"{payment_april:g}억원", delta=f"{payment_april - payment_march:g}억원 (전월 대비)")
    st.metric(label="4월 실제 매출인식액", value=f"{revenue_april:g}억원", delta=f"{revenue_april - revenue_march:g}억원 (전월 대비)")
...
def generate_ybm_anomalies():
    # 현재 시점인 4월로 전표일자 일괄 업데이트 및 'Bizplay' 등 특정 솔루션 명칭 제거
    anomalies = [
        {"전표일자": "2026-04-05", "데이터 구분": "PG사 정산 대사", "이상치 발생 사유": "신용카드 수수료율 오적용 (정산금액 불일치)", "탐지금액": 8342807},
        {"전표일자": "2026-04-06", "데이터 구분": "이러닝 환불", "이상치 발생 사유": "수강진도율 50% 초과건 전액 환불 (규정 위반)", "탐지금액": 4527419},
        {"전표일자": "2026-04-07", "데이터 구분": "출판/교재 물류", "이상치 발생 사유": "교재 물류센터 재고 실사 불일치 (ERP 출고 누락)", "탐지금액": 3072998},
        {"전표일자": "2026-04-08", "데이터 구분": "어학시험 접수", "이상치 발생 사유": "TOEIC 단체접수 할인율 시스템 적용 누락", "탐지금액": 2853339},
        {"전표일자": "2026-04-08", "데이터 구분": "강사료 정산", "이상치 발생 사유": "외주 강사료 원천세(3.3%) 징수액 계산 오류", "탐지금액": 1947389},
        {"전표일자": "2026-04-09", "데이터 구분": "PG 결제 중복", "이상치 발생 사유": "동일 IP, 동일 금액 연속 결제 (중복 결제 의심)", "탐지금액": 1802279},
        {"전표일자": "2026-04-10", "데이터 구분": "법인카드 경비", "이상치 발생 사유": "마케팅 부서 법인카드 고액 결제 (예산 한도 초과)", "탐지금액": 1721441},
        {"전표일자": "2026-04-10", "데이터 구분": "SaaS/구독료", "이상치 발생 사유": "클라우드 인프라 이용료 (환율 변동률 이상치)", "탐지금액": 1670495},
    ]
