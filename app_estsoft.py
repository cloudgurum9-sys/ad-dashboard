...
# 2. 가상 재무 데이터 생성 (이스트소프트 데이터가 기본으로 뜨도록 순서 조정)
@st.cache_data
def load_data():
    data = {
        '연도': ['2024', '2025', '2024', '2025', '2024', '2025'],
        '산업군': ['AI/소프트웨어', 'AI/소프트웨어', '게임업', '게임업', 'IT서비스', 'IT서비스'],
        '기업명': ['이스트소프트', '이스트소프트', '컴투스', '컴투스', 'B서비스', 'B서비스'],
        # 이스트소프트 최근 실적 트렌드 반영 (단위: 억 원)
        '매출액': [888, 1028, 7084, 8560, 10000, 9500],
        '당기순이익': [25, -133, -214, 452, 800, 600],
        '총자산': [1450, 1380, 15200, 14850, 20000, 19000],
        '자기자본': [820, 680, 11300, 11780, 12000, 11500],
        # K-IFRS 공식 명칭 완벽 적용
        '영업활동현금흐름(OCF)': [85, -45, 158, 620, 1200, 900]
    }
    return pd.DataFrame(data)

df = load_data()

# 3. 사이드바 설정 (AI/소프트웨어 및 이스트소프트가 무조건 먼저 뜸)
st.sidebar.header("🔍 분석 필터")

industry_list = df['산업군'].unique().tolist()
# 기본값을 0번 인덱스('AI/소프트웨어')로 고정
